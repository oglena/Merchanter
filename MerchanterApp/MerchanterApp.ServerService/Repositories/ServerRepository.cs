    ...
    public class ServerRepository(MerchanterService merchanterService) : IServerRepository {
        ...
        public async Task<MerchanterServer> StartServer(int _customer_id) {
            ...
            await Task.Factory.StartNew(async () => {
                var customer = await merchanterService.helper.GetCustomer(_customer_id);
                if (customer != null && customer.status) {
                    ...
                    if (is_changed)
                        await merchanterService.helper.SaveCustomer(_customer_id, customer);

                    ProcessStartInfo process = new ProcessStartInfo();
                    process.WorkingDirectory = config["AppSettings:MerchanterServerFilePath"];
                    process.FileName = config["AppSettings:MerchanterServerFileName"] + ".exe";
                    process.Arguments = _customer_id.ToString() /*+ " /accepteula"*/;
                    ...
                    await merchanterService.helper.LogToServer(AppDomain.CurrentDomain.Id.ToString(), "api", "start disabled for user:" + customer.customer_id, customer.customer_id, "server_service");
                }
            });
        ...
        public async Task<MerchanterServer> StopServer(int _customer_id) {
            MerchanterServer stopped_server = new();
            await Task.Factory.StartNew(async () => {
                var processes = Process.GetProcessesByName(config["AppSettings:MerchanterServerFileName"]);
                foreach (var item in processes) {
                    if (int.TryParse(GetCustomerId(item).Split(" ")[0], out int cid)) {
                        try {
                            item.Kill();
                            stopped_server = new MerchanterServer() { customer_id = _customer_id, PID = item.Id, customer = await merchanterService.helper.GetCustomer(cid) };
                        }
                        catch {
                        }
                    }
        ...
        private async Task<List<MerchanterServer>> GetServers() {
            await Task.Factory.StartNew(async () => {
                var processes = Process.GetProcessesByName(config["AppSettings:MerchanterServerFileName"]);
                List<MerchanterServer> servers = [];
                foreach (var item in processes) {
                    if (int.TryParse(GetCustomerId(item).Split(" ")[0], out int cid)) {
                        servers.Add(new MerchanterServer() { PID = item.Id, customer_id = cid, customer = await merchanterService.helper.GetCustomer(cid) });
                    }
                }
