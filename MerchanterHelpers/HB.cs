    ...
    public partial class HB {
        ...
        public HBOrder? GetOrders( string _order_status, int _page_size = 100, int _current_page = 0 ) {
            using( Executioner executioner = new Executioner() ) {
                string? value = executioner.ExecuteHB( base_url + "/packages/merchantid/" + MerchantId + (!string.IsNullOrWhiteSpace( _order_status ) ? "/" + _order_status : "") + "?offset=" + _current_page.ToString() + "&limit=" + _page_size.ToString(),
                    RestSharp.Method.Get, null, Token );
                if( !string.IsNullOrWhiteSpace( value ) && value.Trim() != "[]" ) {
                    return Newtonsoft.Json.JsonConvert.DeserializeObject<HBOrder>( value );
                }
                else return null;
            }
        }
