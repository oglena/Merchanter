	...
	try {
		...
		if( Helper.global != null && main_loop.DoWork() ) {
			if( customer.product_sync_status && !customer.is_productsync_working ) {
				...
				Helper.global.settings.company_name + " PRODUCT SYNC COMPLETED", ConsoleColor.Blue );
			}
			if( customer.order_sync_status && !customer.is_ordersync_working ) {
				WriteLogLine( "[" + DateTime.Now.ToString() + "] " + customer_id.ToString() + "-" +
				Helper.global.settings.company_name + " ORDER SYNC COMPLETED", ConsoleColor.Blue );
			}
			if( customer.xml_sync_status && !customer.is_xmlsync_working ) {
				WriteLogLine( "[" + DateTime.Now.ToString() + "] " + customer_id.ToString() + "-" +
				Helper.global.settings.company_name + " XML SYNC COMPLETED", ConsoleColor.Blue );
			}
			if( customer.invoice_sync_status && !customer.is_invoicesync_working ) {
				WriteLogLine( "[" + DateTime.Now.ToString() + "] " + customer_id.ToString() + "-" +
				Helper.global.settings.company_name + " INVOICE SYNC COMPLETED", ConsoleColor.Blue );
			}
			if( customer.notification_sync_status && !customer.is_notificationsync_working ) {
				WriteLogLine( "[" + DateTime.Now.ToString() + "] " + customer_id.ToString() + "-" +
				Helper.global.settings.company_name + " NOTIFICATION SYNC COMPLETED", ConsoleColor.Blue );
			}
		}
	}
	catch( Exception _ex ) {
