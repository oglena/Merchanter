...
namespace ApiService.Controllers {
    ...
    public class CategoryController(ICategoryService categoryService, ICatalogService catalogService) : ControllerBase {
        ...
        public async Task<ActionResult<BaseResponseModel<List<Category>>>> GetCategories([FromBody] ApiFilter? _filters) {
            if (int.TryParse(HttpContext.User.FindFirst("customerId")?.Value, out int customer_id) && customer_id > 0) {
                ...
                _filters.ExtendedQueryResponses = await catalogService.GetExtendedQuery(customer_id, _filters, typeof(Category));
